...
//
void Periodical :: CheckoutLink (Library * M) //const 
{
    for (int i = 0; i < Issues.size(); i++) {
        debug << "does memID " << M->GetID() << " == coBy " << Issues[i].CheckedOutByStr << endl;
        if (Issues[i].CheckedOutByStr == M->GetID()) {
            Issues[i].CheckedOutBy = M;
            return;
        }
    }
}
...
//
void Periodical :: Checkout (Library * M, string str, int count) //const
{
    Issues[count].CheckedOutByStr = str;
    Issues[count].CheckedOutBy = M;
} 

//
void Periodical :: Return (int count)     
{
    Issues[count].CheckedOutByStr = "NONE";
    Issues[count].CheckedOutBy = NULL;
}
