...
//
void Periodical :: SetCheckedOut (Date & D)
{
    I.CheckedOut = D;
}
...
//
void Periodical :: CheckoutLink (vector <Library *> vM, vector <Library *> vP) //const 
{
    debug << "Periodical ID = " << GetID() << endl;

    for (int j = 0; j < vM.size(); j++) {
        for (int k = 0; k < Issues.size(); k++) {
            debug << "\tcoID = " << Issues[k].CheckedOutByStr << endl;
            if (vM[j] != NULL && Issues[k].CheckedOutByStr == vM[j]->GetID())
            {
                debug << "\t" << Issues[k].CheckedOutByStr << " ?= " << vM[j]->GetID() << endl;
                Library *m = vM[j]; // Member *m = vM[j];
                Issues[k].CheckedOutBy = m;
                if (Issues[k].CheckedOutBy != m)
                    debug << "shit be broken you!\n";
                debug << "\t\tit does\n";
                break;
            }
            else if (vM[j] == NULL) {
                debug << "Member at " << j << " is NULL\n";
            }
        }
    }
}
